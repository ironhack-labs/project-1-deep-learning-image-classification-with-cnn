...
def load_cifar10_batches(path):
    batches = [unpickle(f'{path}/data_batch_{i}') for i in range(1, 6)]
    data = np.vstack([batch[b'data'] for batch in batches]).reshape(-1, 3, 32, 32)  # Reshape
...
def train_and_evaluate(model, train_loader, test_loader, criterion, optimizer, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        ...
        train_accuracy = accuracy_metric.compute() * 100
...
# Train the model
model = train_and_evaluate(model, train_loader, test_loader, criterion, optimizer)
